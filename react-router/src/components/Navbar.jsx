...
const Navbar = () => {
  ...
  return (
    <div className='md:px-20 px-10  fixed z-[999] py-2 md:py-8 w-full font-["Neue Montreal"] flex justify-between items-center md:bg-transparent'>
      ...
      <div className='hidden md:flex gap-9'>
        {["Home", "About", "Contact"].map((item, index) => (
          <Link to={item} key={index} className={`text-md capitalize font-light ${index === 4 && "ml-40"}`}>{item}</Link>
        ))}
      </div>
      ...
      {isOpen && (
        ...
        <div className='absolute top-16 left-0 w-full bg-gray-900 md:hidden flex flex-col gap-5 items-center py-4 shadow-md'>
          {["Service", "Our Work", "About us", "Insights", "Contacts"].map((item, index) => (
            <a key={index} className={`text-md capitalize font-light ${index === 4 && "ml-40"}`}>{item}</a>
          ))}
        </div>
      )}
    </div>
